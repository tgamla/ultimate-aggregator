...
describe('Aggregate functions', () => {
    ...
    describe('FIRST', () => {
        ...
        it('Get first invoice with 7th position from first 10 rows', () => {
            var result = new ua.Query()
                .select('FIRST(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get first invoice with 7th position ordered by amount for first 10 rows', () => {
            var result = new ua.Query()
                .select('FIRST(row.positions[6] ? row.positions[6].amount : null)OVER()ORDER_BY()')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get first invoice with 7th position ordered by amount descending for first 10 rows', () => {
            var result = new ua.Query()
                .select('FIRST(row.positions[6] ? row.positions[6].amount : null)OVER()ORDER_BY(DESC)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get first invoiceNo ordered by Region and invoiceNo descending for first 10 rows', () => {
            var result = new ua.Query()
                .select('FIRST(row.invoiceNo)OVER()ORDER_BY(row.departmentRegion, DESC)')
                .from(testData.slice(0, 10))
                .toList();
    ...
    describe('LAST', () => {
        ...
        it('Get last invoice with 7th position ordered by amount from first 10 rows', () => {
            var result = new ua.Query()
                .select('LAST(row.positions[6] ? row.positions[6].amount : null)OVER()ORDER_BY()')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get last invoice with 7th position ordered by amount descending from first 10 rows', () => {
            var result = new ua.Query()
                .select('LAST(row.positions[6] ? row.positions[6].amount : null)OVER()ORDER_BY(DESC)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get last invoiceNo ordered by Region and invoiceNo descending from first 10 rows', () => {
            var result = new ua.Query()
                .select('LAST(row.invoiceNo)OVER()ORDER_BY(row.departmentRegion, DESC)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
    ...
    describe('NTH', () => {
        ...
        it('Get 3rd invoice with 7th position ordered by amount from first 10 rows', () => {
            var result = new ua.Query()
                .select('NTH((row.positions[6] ? row.positions[6].amount : null), 3)OVER()ORDER_BY()')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get 3rd invoice with 7th position ordered by amount descending from first 10 rows', () => {
            var result = new ua.Query()
                .select('NTH((row.positions[6] ? row.positions[6].amount : null), 3)OVER()ORDER_BY(DESC)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get 3rd invoiceNo ordered by Region and invoiceNo descending from first 10 rows', () => {
            var result = new ua.Query()
                .select('NTH(row.invoiceNo, 3)OVER()ORDER_BY(row.departmentRegion, DESC)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
    ...
    describe('CONCAT', () => {
        ...
        it('Get concatenated invoices OVER region from first 10 rows', () => {
            var result = new ua.Query()
                .select('CONCAT(row.invoiceNo) OVER (row.departmentRegion)')
                .from(testData.slice(0, 10))
                .toList();
        ...
        it('Get concatatenation of 7th position of each invoice from first 10 rows', () => {
            var result = new ua.Query()
                .select('CONCAT(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get concatenation of invoicesNo ordered by VALUE from first 10 rows', () => {
            var result = new ua.Query()
                .select(ua.group('CONCAT(row.invoiceNo)OVER()ORDER_BY()').by('row.departmentRegion'))
                .from(testData.slice(0, 10))
                .toList();
        ...
        it('Get concatenation of invoicesNo ordered by region and VALUE descending from first 10 rows', () => {
            var result = new ua.Query()
                .select(ua.group('CONCAT(row.invoiceNo + ": " + row.departmentRegion )OVER()ORDER_BY(row.departmentRegion, DESC)').by('row.departmentRegion'))
                .from(testData.slice(0, 10))
                .toList();
    ...
    describe('MIN', () => {
        ...
        it('Get minimum invoice amount over region grouped by region and department from first 10 rows', () => {
            var result = new ua.Query()
                .select({
                    ...
                    minAmount: 'row.departmentRegion + ": " + MIN(row.invoiceAmount) OVER (row.departmentRegion)'
                })
                .from(testData.slice(0, 10))
        ...
        it('Get minimum amount of 7th invoice position from first 10 rows', () => {
            var result = new ua.Query()
                .select('MIN(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
    ...
    describe('MAX', () => {
        ...
        it('Get maximum invoice amount over region grouped by region and department from first 10 rows', () => {
            var result = new ua.Query()
                .select({
                    department: 'row.department',
                    maxAmount: 'row.departmentRegion + ": " + MAX(row.invoiceAmount) OVER (row.departmentRegion)'
                })
                .from(testData.slice(0, 10))
                .groupBy([ 'row.departmentRegion', 'row.department' ])
                .toList();
        ...
        it('Get maximum amount of 7th invoice position from first 10 rows', () => {
            var result = new ua.Query()
                .select('MAX(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
    ...
    describe('SUM', () => {
        it('Get sum of invoices amount grouped by region from first 10 rows', () => {
            var result = new ua.Query()
                .select('SUM(row.invoiceAmount).toFixed(2)')
                .from(testData.slice(0, 10))
                .groupBy('row.departmentRegion')
                .toList();
        ...
        it('Get sum of invoices amount OVER region with region grouped by region, department from first 10 rows', () => {
            var result = new ua.Query()
                .select({
                    region: 'row.departmentRegion',
                    invoicesAmount: 'SUM(row.invoiceAmount) OVER (row.departmentRegion).toFixed(2)'
                })
                .from(testData.slice(0, 10))
                .groupBy(['row.departmentRegion', 'row.department'])
                .toList();
        ...
        it('Get sum of 7th position on each invoice from first 10 rows', () => {
            var result = new ua.Query()
                .select('SUM(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
    ...
    describe('AVG', () => {
        ...
        it('Get avarage invoices amount OVER region with region grouped by region and department from first 10 rows', () => {
            var result = new ua.Query()
                .select({
                    region: 'row.departmentRegion',
                    avarageAmount: 'AVG(row.invoiceAmount)'
                })
                .from(testData.slice(0, 10))
                .groupBy('row.departmentRegion')
                .toList();
        ...
        it('Get avarage of 7th position on each invoice from first 10 rows', () => {
            var result = new ua.Query()
                .select('AVG(row.positions[6] ? row.positions[6].amount : null)')
                .from(testData.slice(0, 10))
                .groupBy()
                .toList();
        ...
        it('Get count of invoices OVER region with department grouped by region and department from first 10 rows', () => {
            var result = new ua.Query()
                .select({
                    ...
                    invoicesCount: 'COUNT(ALL) OVER (row.departmentRegion)'
                })
                .from(testData.slice(0, 10))
                .groupBy(['row.departmentRegion', 'row.department'])
                .toList();
    ...
    describe('Post processing', () => {
        ...
        it('CONCAT OVER, CONCAT, CONCAT(AVG)', () => {
            var result = new ua.Query()
                .select({
                    departmentsAvg: 'CONCAT(AVG(row.invoiceAmount).toFixed(2))',
                    invoices: 'CONCAT(row.invoiceNo) OVER (row.departmentRegion)',
                    departments: 'CONCAT(row.department)'
                })
                .groupBy(['row.departmentRegion', 'row.department'])
                .from(testData.slice(0, 10))
                .toList();
        ...
        it('CONCAT without query grouping + inner groups', () => {
            var result = new ua.Query()
                .select({
                    regions: ua.group({
                        invoicesByRegion: 'CONCAT(row.invoiceNo)',
                        invoicesByAccount: ua.group('CONCAT(row.invoiceNo) OVER (row.accountId)').by(['row.y','row.accountId'])
                    }).by('row.departmentRegion'),
                    invoices: 'CONCAT(row.invoiceNo + ": " + SUM(row.invoiceAmount).toFixed(2)) OVER (row.x)',
                    departments: 'CONCAT(row.department) ORDER_BY (row.department)',
                    departmentsByRegions: 'CONCAT(row.department) OVER(row.departmentRegion) ORDER_BY (row.department)'
                })
                .groupBy()
                .from(testData.slice(0, 10))
                .toList();
